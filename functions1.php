...
function register(){
	...
	global $db, $errors, $username, $firstname, $lastname;
	...
	if (count($errors) == 0) {
		$password = $last_name;//encrypt the password before saving in the database

		if (isset($_POST['user_type'])) {
			$user_type = e($_POST['user_type']);
			$query = "INSERT INTO tbl_admin (employee_id, email, user_type, password) 
					  VALUES('$username', '$email', '$user_type', '$password')";
			mysqli_query($db, $query);
			$_SESSION['success']  = "New user successfully created!!";
			header('location: home4admin.php');
		}else{
			$query = "INSERT INTO tbl_admin (employee_id,first_name,last_name, password) 
